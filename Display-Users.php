...
<?php
	...
	*************************************************************************************/
    ...
    $q = "SELECT * FROM myusers WHERE active='y' ORDER BY $sort_type";    # we put query in q
    $r = mysqli_query ($dbc, $q);   # this runs query, using $dbc

    // start a table and create a header
    echo "<table border=1>";
    echo "<tr>";
        echo "<th style = 'padding-left: 5px'>User ID </th>";
        echo "<th style = 'padding-left: 5px'>Username </th>";
    echo "<th style = 'padding-left: 5px'>Date of Birth </th>";
    echo "<th style = 'padding-left: 5px'>Date of Hire </th>";
    echo "<th style = 'padding-left: 5px'>User Address </th>";
    echo "<th style = 'padding-left: 5px'>Password </th>";
    echo "<th style = 'padding-left: 5px'>Password Change </th>";
    echo "<th style = 'padding-left: 5px'>Email </th>";
    echo "<th style = 'padding-left: 5px'>Name </th>";
    ...
    if ($r ) {
        while ($row = mysqli_fetch_array ($r, MYSQLI_NUM))
        {
            echo "<tr> <td> $row[0] </td> <td> $row[1] </td><td> $row[2] </td><td> $row[3] </td><td> $row[4] </td>" . "
                                <td> $row[5] </td><td> $row[6] </td><td> $row[7] </td><td> $row[8] </td>  <td> $row[9] </td> <td> <a href='Delete_Users.php" . $row[10] . "'> DELETE</a> </td> ";
            echo "</tr>";
        }
    }
    ...
    echo "<form action = 'Display-Users.php' method = 'POST'>";
	...
	echo "<br>";
	echo "<br>";
	echo "<form action = 'Input-User.php'>";
